...
- YES **Multi-result with per-result grades.** `/locate` returns `results: [{anchor,file,lines,snippet,score,grade}]` best-first (default top 3). Each graded 1-10 by the judge (one batched call). `found` = best clears the gate. Lets Claude see "this 10/10, that 8/10 indirectly".
- YES **Security / input hardening** (`src/security.py`). Request validation: `query` max 500 chars, `lib`/`version` restricted to `[A-Za-z0-9._-]` (HTTP 422 otherwise). Query sanitized (control chars stripped). Prompt-injection defense: rewriter + judge prompts wrap the query in `<question>` tags and instruct the model to treat it as data, never follow embedded instructions. Outputs clamped/validated (grade 1-10, plan schema) -> tiny blast radius. No egress + no tools to the LLM -> nothing to exfiltrate.
- YES **On-demand fetch (`POST /add {lib, version?}`).** Claude can request a library; the service fetches + indexes it (append) and reloads. Connected mode only — **403 in air-gap** (there the indexer adds docs at build time). Validated lib/version.
- DATA **Token benchmark (`src/bench_tokens.py`).** Complex query on the real FastAPI README (~5,776 est. tokens for the whole page): Aegis snippet ~182 tokens (**~32x fewer**), pointer-only ~18 tokens (**~321x fewer**). Corpus traversal is local = 0 Claude tokens. Real "search the web myself" pulls several pages -> savings larger.
- FINDING **LLM judge with small models DOESN'T WORK — judge default OFF.** Tested 1.5b and 3b extensively (JSON, plain number, chain-of-thought): both rate a perfect match (StreamingResponse for "how to stream") as 1-2/10; 3b even *reasons* that the perfect snippet "does not answer the question". Small local models (<=3b) can't do calibrated relevance grading. **Decision:** `AEGIS_JUDGE` defaults OFF; rely on the **cosine score** (ranked correctly: StreamingResponse 0.839) + **Claude's own judgment** — it reads the tiny snippet and is a far stronger judge than any small local model, and it's cheap precisely because the snippet is small. Judge stays opt-in for a 7b+ model. This is also the answer to "how does Claude validate": Claude is the validator; `score` is a hint, not proof.
